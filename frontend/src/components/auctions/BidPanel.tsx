...
export const BidPanel = ({ auctionId }: BidPanelProps) => {
  ...
  const onBid = async (data: BidFormData) => {
    ...
    try {
      ...
      toast.success(`Bid of ${formatPrice(data.amount)} placed!`);
      reset();
    } catch (err: any) {
  ...
  return (
    <div className="card-gold p-5 space-y-4" role="form" aria-label="Place a bid">
      {/* Current Bid Display */}
      <div className="text-center">
        <p className="text-muted text-xs uppercase tracking-wide mb-1">Current Highest Bid</p>
        <p className="text-3xl font-display font-bold text-primary">{formatPrice(currentBid)}</p>
        <p className="text-xs text-muted mt-1">Min next bid: <span className="text-white">{formatPrice(minBid)}</span></p>
      </div>

      <form onSubmit={handleSubmit(onBid)} className="space-y-3">
        <div>
          <label htmlFor="bid-amount" className="text-xs text-muted mb-1 block">Your Bid (₹)</label>
          <input
            id="bid-amount"
            type="number"
